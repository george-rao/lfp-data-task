...
```{r}
women_over_25 |> plot_income_over_time_by_group(income)
women_over_25 |> plot_income_over_time_by_group(income, race)
women_over_25 |> plot_income_over_time_by_group(income, age)
```
...
```{r}
women_over_25 |> plot_income_over_time_by_group(incss)
women_over_25 |> plot_income_over_time_by_group(incss, race)
women_over_25 |> plot_income_over_time_by_group(incss, age)
```
...
women_over_25 |>
  ...
  summarise(
    .by = year,
    mean_inctot = weighted.mean(inctot[!is.na(inctot)], wgt[!is.na(inctot)]),
    mean_income = weighted.mean(income[!is.na(income)], wgt[!is.na(income)]),
    mean_incss = weighted.mean(incss[!is.na(incss)], wgt[!is.na(incss)]),
    median_inctot = Quantile(inctot[!is.na(inctot)], wgt[!is.na(inctot)], 0.5),
    median_income = Quantile(income[!is.na(income)], wgt[!is.na(income)], 0.5),
    median_incss = Quantile(incss[!is.na(incss)], wgt[!is.na(incss)], 0.5)
  ) |>
  pivot_longer(
    cols = mean_inctot:median_incss,
    names_sep = "_",
    names_to = c("statistic", "income_type"),
    values_to = "income"
  ) |>
  ggplot(aes(x = year, y = income, color = income_type, linetype = statistic)) +
  geom_line(alpha = 0.75)
```

This chart shows the average income earned of each type for individuals actually earning that kind of income (individuals for which the given type of income is zero are excluded). The average and median for all three kinds of income are increasing over time.
